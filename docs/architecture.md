...
flowchart TB
    ...
    class REPO git;
    class INSTALL,SECRET,TOKEN,ALERTS,TGCONF,SYSCTL install;
...
9. При заданных Bot API секретах watchdog шлёт Telegram alert при переходе в `unhealthy` и recovery при возврате в `healthy`.
10. Браузер опрашивает API каждые `5` секунд и обновляет экран.
11. Локальный `scripts/mtproxy-metric.sh` при необходимости читает ту же метрику через `SSH`.
...
| `Telegram Bot API` | канал уведомлений о проблемах и recovery |
...
| `/etc/mtproxy/alert-*` | Bot API token и chat id для alerting |
